        ...
        <div key={item.apiId} id={item.apiId} className={'media-item ' + (item.active ? "media-item--active" : item.complete ? 'media-item--complete' : '')}>
          ...
          {item.firstName ?
            <p className="media-item__friend">
              <strong>{item.firstName} {item.lastName}: {item.username}</strong>
            </p>
          : ""}
          ...
          : props.resultType === "results" ?
            <div className="media-item__buttons media-item__buttons--results">
              <button onClick={() => props.handleAddFriend(item.apiId)} className="btn btn-request">Add Friend <i className="icon icon-bookmark-empty"></i></button>
            </div>
          : ""}
